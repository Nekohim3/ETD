...
namespace RogueSharp
{
   ...
   public class Map<TCell> : IMap<TCell> where TCell : ICell
   {
      ...
      public IEnumerable<TCell> GetAdjacentCells( int xCenter, int yCenter, bool includeDiagonals )
      {
         ...
         if (includeDiagonals)
         {
             if (rightX < Width && topY >= 0 && _cells[xCenter, topY].IsWalkable && _cells[rightX, yCenter].IsWalkable)
             {
                 yield return GetCell(rightX, topY);
             }

             if (rightX < Width && bottomY < Height && _cells[xCenter, bottomY].IsWalkable && _cells[rightX, yCenter].IsWalkable)
             {
                 yield return GetCell(rightX, bottomY);
             }

             if (leftX >= 0 && topY >= 0 && _cells[xCenter, topY].IsWalkable && _cells[leftX, yCenter].IsWalkable)
             {
                 yield return GetCell(leftX, topY);
             }

             if (leftX >= 0 && bottomY < Height && _cells[xCenter, bottomY].IsWalkable && _cells[leftX, yCenter].IsWalkable)
             {
